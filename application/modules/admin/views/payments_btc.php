...
<?php 
    $count = 1;
    if(isset($btc_transactions) && is_array($btc_transactions) && count($btc_transactions)>0 ) { 
        foreach ($btc_transactions as $key => $value) { ?>
        <tr>
          <td><?php echo $count; ?></th>
          <td><a target="_blank" href="https://blockchain.info/tx/<?php echo $value['txid']; ?>"><?php echo substr($value['txid'],0,10); ?></a></td>
